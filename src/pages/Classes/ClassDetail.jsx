...
const ClassDetail = () => {
  ...
  const calcScore = (student) => {
    const discipline = student.disciplineScore ?? 0;
    const reward = student.rewardScore ?? 0;
    return discipline < 10 ? Math.min(discipline + reward, 10) : discipline;
  };

  const calcRemainingBonus = (student) => {
    const discipline = student.disciplineScore ?? 0;
    const reward = student.rewardScore ?? 0;
  ...
  const fetchClassDetail = async () => {
    ...
    try {
      ...
      const sorted = [...data].sort((a, b) => calcScore(b) - calcScore(a));
      setClassData(sorted);
    } catch (err) {
  ...
  const getStatus = (score) => {
    if (score <= 4)
      return {
        text: "Sariq ro'yhat",
        className: "bg-yellow-100 text-orange-600",
      };
    if (score <= 6)
      return {
        text: "Ogohlantirish",
  ...
  return (
    <div className="px-4 py-6 sm:px-6 sm:py-8 md:px-8 md:py-10">
      ...
      <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-slate-800 mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3">
        <GraduationCap
          size={26}
          className="text-indigo-600 shrink-0 sm:hidden"
        />
        <GraduationCap
          size={32}
          className="text-indigo-600 shrink-0 hidden sm:block"
        />
      ...
      <div className="grid grid-cols-1 gap-3 sm:gap-4">
        ...
        {classData.map((student, index) => {
          const totalScore = calcScore(student);
          const remainingBonus = calcRemainingBonus(student);
            ...
            >
              ...
              <div className="hidden sm:flex items-center justify-between gap-3">
                ...
                <p className="text-2xl md:text-3xl text-black font-bold shrink-0">
                  {totalScore}
                  <span className="text-sm md:text-base text-slate-400 font-medium ml-1">
                    ball
                  </span>
                </p>
                {remainingBonus > 0 && (
                  <div className="flex items-end">
                    <p className="text-3xl text-indigo-500 font-bold mr-0">
                      +{remainingBonus}
                    </p>
                    <span className="text-sm md:text-base text-indigo-500 font-medium ml-1">
                      ball
                    </span>
                  </div>
                )}
                ...
                <span
                  className={`rounded-full px-3 md:px-4 py-1 text-sm font-bold shrink-0 ${status.className}`}
                >
                  {status.text}
                </span>
          ...
          <div className="bg-white w-full sm:max-w-lg rounded-t-[2.5rem] sm:rounded-[3rem] p-6 sm:p-10 shadow-2xl">
            ...
            >
              {[
                ...
                ({ label, value, setter, placeholder, icon: Icon, type }) => (
